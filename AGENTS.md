...
- `packages/compiler-check/`: pure check reports, baseline comparison, refusal ownership classification, and policy evaluation.
...
- `compiler-check` derives deterministic reports, baselines, cascades, and caller-selected policy results from package compilation reports without invoking a compiler or filesystem.
...
- `npm run readiness:corpus`: report what fraction of the full Flight SDK corpus each target emits, and rank refusal families by the modules they block directly and transitively. This is the number `readiness` cannot see: the golden corpus is a set of probes for behavior the compiler already has, so it reports a share near 100% while most of the SDK refuses. The corpus can only be generated downstream, because the request needs a target repository's package graph, package targets, and binding profiles; this reads the ledger that run leaves behind (`--corpus=<directory>` to point elsewhere), states the compiler revision that produced it and how far this checkout has moved past it, and never runs the C++ toolchain. A reporting instrument; nothing gates on it.
- `npm run readiness:issues`: report how many _distinct issues_ the SDK corpus holds, who owns each one, and the entity/host/node foundations individually. `readiness:corpus` ranks families by the modules they block, which answers "which rule is expensive"; this answers "how much work is left and whose is it", because a module count moves only when a fix happens to unblock a whole module, so it reads as progress that is not there and hides progress that is. Its two fingerprints change only when the _set_ of distinct issues changes, so a change that swaps one refusal for another is visible even when every total holds. Module identities that only inherit a dependency's refusal are reported as cascades rather than counted as issues. Reads the same ledgers `readiness:corpus` reads and performs no generation or compilation; prints by default, `--out=<file>` also writes, `--foundation=<package|module>` adds an identity. **The compiler records only the first refusal per module, so the issue count is a floor, not a total.** A reporting instrument; nothing gates on it.
...
Coverage thresholds are enforced ratchets, not aspirational targets: the current 89.8% branches, 97.2% functions, 95.4% lines, and 93.2% statements floors sit immediately below the combined compiler baseline (89.90 / 97.31 / 95.51 / 93.34 on 2026-09-24) so regressions fail promptly. Maintain or raise them as exercised compiler surface grows. Lowering a threshold requires an explicit architectural justification, which is recorded beside the numbers: the 2026-09-24 floors follow 670 commits and 74,407 added lines of mostly engine surface, where 87-92% of the misses sit in the C++/Haxe/Rust backends and semantic lowering, and no part of the measurement changed with them.
