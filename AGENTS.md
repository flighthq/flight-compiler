...
- `flight-cpp/`: self-contained C++20 runtime incubator, CMake package, tests, and compiler-integration contract.
...
- `npm run compile:check`: hand every emitted fixture to the target's own compiler. C++ syntax-checks each header with `g++`; Haxe type-checks against `golden/support`, which supplies the runtime contract and sibling modules a one-module fixture cannot; Rust builds the runtime as a crate and checks each fixture against it. A toolchain that is not installed is reported and skipped rather than failed.
- `npm run cpp:build:check`: require the standalone CMake and Bazel projects to agree on public headers, runtime sources, executable tests, and benchmarks; pin the Bazel release and CI bootstrap; and preserve explicit platform/toolchain selection without requiring Bazel on the machine running repository metadata checks.
...
Coverage thresholds are enforced ratchets, not aspirational targets: the current 94% branches, 98% functions, 97% lines, and 96% statements floors sit immediately below the measured baseline (94.84 / 99.05 / 97.84 / 96.93 on 2026-09-08) so regressions fail promptly. Maintain or raise them as exercised compiler surface grows. Lowering a threshold requires an explicit architectural justification.
