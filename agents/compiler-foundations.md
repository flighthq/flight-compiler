...
| `compiler-inventory` | Keep independent as the first read-only composition above identity. | Package discovery yields validated, portable, deterministically ordered manifest, dependency, bin, production-import, and host-module facts with tagged boundary failures; export graphs, runtime bindings, config failures, and Git identity are covered, while evidence-driven exclusion parity remains open. |
