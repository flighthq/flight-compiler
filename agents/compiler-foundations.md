...
The Rust backend maps packages to crate modules, avoids Rust keywords, and normalizes case. Identity-based declarations and references preserve binding provenance. Exact ownership evidence covers mutability, use counts, rebinding versus referent mutation, storage classification, cross-capture and cross-carrier uses, and suspension boundary crossings without choosing borrowing, cloning, reference counting, or pinning. Inventory-routed structural preflight and direct-versus-lowered operator decisions are deterministic. The small golden corpus remains at 197 of 200 emitted fixtures, but a 2026-09-13 `@flighthq/types` package-graph census without downstream host bindings emits 306 of 977 modules (31.3%). Of its 164 direct refusals, 96 require target-native host bindings; the remaining compiler-owned concentrations are heterogeneous unions, unresolved intersections, two open utility filters, one namespace, and one nonlocal ambient interface heritage. Rust now resolves extensionless and directory imports, emits module facades, separates multi-package output by crate, materializes closed object utilities and intersections, resolves imported tagged-union alternatives, erases same-domain literal unions, and accepts a versioned host-binding manifest. A full Flight SDK rerun remains necessary after a real Rust host manifest and target-neutral conditional/mapped-type lowering clear those dependency gates. Ownership analysis has zero unreached arms; the wider emitter remains the principal incomplete surface.
