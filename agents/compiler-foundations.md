...
| `compiler-inventory` | Keep independent as the first read-only composition above identity. | Package discovery now yields validated, portable, deterministically ordered manifest, dependency, and bin facts with tagged failure codes; export graphs, runtime bindings, config failures, and Git identity are covered, while production-import facts and exclusion parity remain open. |
