...
| `compiler-inventory` | Keep independent as the first read-only composition above identity. | Package discovery yields validated, portable, deterministically ordered manifest, dependency, bin, production-import, host-module, and evidence-backed tooling-exclusion facts; every manifest, project, Git, export-graph, source-resolution, runtime-classification, SDK, and exclusion failure is tagged for message-independent handling, while static semantic facts and host-endpoint coverage remain open. |
