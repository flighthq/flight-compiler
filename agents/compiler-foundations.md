...
The package also owns a versioned completion-set algebra for normal, break, continue, return, and throw routes. Break and continue retain exact optional target identity. Construction validates, deduplicates, freezes, and orders routes by the shared code-unit canonical form; alternative composition is commutative and idempotent; sequential composition is associative, treats normal completion as the only route into the next statement, preserves earlier abrupt routes, uses normal as its empty-list identity, and validates even unreachable tail input. The lowering package now derives variable-initialization completion keys from the shared vocabulary. `finally` replacement remains the next primitive.
