...
Status: narrow target-neutral module linking, evaluation, and facade bedrock.

The package accepts an explicit module set, ordered entry set, and ordered resolved dependency edges rather than consulting a filesystem or target resolver. It separates link, instantiation, dependency evaluation, and body evaluation; computes dependency-first strongly connected groups without rejecting cycles; and retains the exact import request order supplied by the linking boundary. Every value import is a read-only live alias to its dependency cell. Local `var` and TypeScript enum bindings are available as `undefined` after instantiation and assigned during evaluation; functions are available after instantiation; `let`, `const`, and classes remain temporal until their exact evaluation step. Destructuring leaves share one initialization step, local mutability remains explicit, and every executable step states that abrupt completion stops module evaluation. Type-only imports and re-exports remain linked for facade resolution without entering runtime evaluation groups; side-effect imports and value re-exports do enter those groups.

Modifier exports are materialized as exact local routes, including every destructuring leaf, rather than remaining an emitter-only boolean. Facade lowering resolves independent type and value slots through local bindings, default expressions, imported aliases, named and namespace re-exports, and cyclic star graphs. Explicit names override stars, `default` never crosses a star, identical diamond resolutions coalesce, and distinct star resolutions refuse as ambiguous. Every slot retains route hops to its terminal live binding, expression, or namespace, while its public identity remains independent of that implementation route. A separate predicate distinguishes declaration-reflecting exports that existing backends already emit from aliases and cross-module routes that require facade lowering.

Duplicate modules, entries, dependencies, bindings, and public slots; malformed identities or graph shapes; missing, unexpected, or unreachable edges; unresolved routes; star ambiguity; mutability contradictions; multiple default expressions; lost default-expression ordering; and unsupported top-level `await` fail through stable tagged data. Module-array order is irrelevant, dependency and entry order are deliberate semantics, caller inputs remain unchanged, results are deeply immutable, and isolated coverage reaches every implementation arm. Haxe and Rust syntax remain outside this package.
...
Status: Rust naming, ownership evidence, and source emission are explicit under one target policy boundary, but golden coverage is not package-graph maturity.

The Rust backend maps packages to crate modules, avoids Rust keywords, and normalizes case. Identity-based declarations and references preserve binding provenance. Exact ownership evidence covers mutability, use counts, rebinding versus referent mutation, storage classification, cross-capture and cross-carrier uses, and suspension boundary crossings without choosing borrowing, cloning, reference counting, or pinning. Inventory-routed structural preflight and direct-versus-lowered operator decisions are deterministic. The small golden corpus now emits 198 of 200 fixtures, but a 2026-09-13 `@flighthq/types` package-graph census without downstream host bindings emits 326 of 977 modules (33.4%). Of its 108 direct refusals, 97 require target-native host bindings. The remaining 11 compiler-owned modules reduce to two target-neutral gaps: eight `NodeOf<Traits> & Trait` modules need generic row-composition evidence, while `CapacitorHost`, `ElectronHost`, and `CreateTextureOptions` depend on exported conditional/mapped aliases that semantic lowering does not yet represent. Rust now resolves extensionless and directory imports, emits module facades, separates multi-package output by crate, materializes closed object utilities and narrowed intersections, resolves imported and anonymous tagged-union alternatives, represents callable objects, erases scalar brands and same-domain literal unions, lowers numeric enum value namespaces, accepts materialized ambient utility heritage, and accepts a versioned host-binding manifest. Its typed arrays, `ArrayBuffer`, and `DataView` use one runtime-owned byte-storage representation: indexed and compound writes coerce per element kind, buffer-backed constructors and `subarray` share storage with distinct view identity, `slice` copies storage, and `.buffer` recovers the shared source identity without exposing storage fields. Typed-array constructor arities zero through three and DataView arities one through three are elected; byte metadata and endian-aware integer and floating-point access use the runtime contract. A full Flight SDK rerun remains necessary after a real Rust host manifest and target-neutral conditional/mapped-type and generic-row lowering clear those dependency gates. Ownership analysis has zero unreached arms; the wider emitter remains the principal incomplete surface.
...
The command-line package parses `argv`, validates target-specific and source-generating options, dispatches compilation through the orchestration pipeline, and formats a grouped report of emitted and refused modules. Every option, its validation boundary, and the usage text are end-to-end tested. Mutation testing kills 64 of 67 mutants (95.5% kill rate); the three survivors are a comparison-chain tiebreaker over structurally unique map keys (unreachable equality), and two entry-point guard equivalents. Eight real mutants were killed through targeted tests: the `--runtime-header` flow through to emitted C++ output, include-path special character and empty-path rejection, refusal column/line conditional inclusion, the report sample-size truncation boundary, and the runtime-profile error-message branch.
...
| `compiler-module` | Keep independent as the target-neutral module linking, instantiation, evaluation, and facade boundary. | Complete link edges with runtime-versus-type-only evaluation identity, entry-rooted dependency-first SCCs, cycle preservation, function/var/enum instantiation, lexical temporal access, destructuring initialization, declaration-modifier export fidelity, stable public-slot identity, live local/import/re-export routes, namespace and default routes, explicit-over-star precedence, diamond coalescing, star ambiguity, tagged malformed or unresolved failures, deterministic deep immutability, and zero unreached arms are covered; target facade representation remains open. |
...
| `compiler-ir-traversal` | Keep independent as the dependency-floor read-only IR observation boundary. | Typed structural walks over modules, declarations, statements, expressions, and types without embedding analysis policy; consumption by closure, structural, module, and task packages demonstrates independent utility. |
| `compiler-command-line` | Keep together as the CLI entry point and argument contract. | Argument validation, target dispatch, per-module graph compilation, report formatting, and usage generation are end-to-end tested; validation boundary and report ordering branches are covered. |
| `compiler-backend-cpp` | Keep together; naming, identity, reference representation, external bindings, and source emission are focused sibling sources under one C++ target policy boundary. | Package/module identity, C++ namespace and file naming, keyword avoidance, dual standard-library and flight-cpp runtime profiles, external binding manifests, reference representation planning, emission session graph-wide analysis, and 191/192 golden fixture emission are covered; WeakMap key reference representation remains the sole blocking rule. |
...
2. **Complete:** derive module-facade lowering from stable public-slot identities and the shared module-evaluation plan.
