...
Class initialization now has a versioned target-neutral plan in `compiler-lowering`. Every declared field retains its source-array identity and exact initializer-versus-implicit-undefined decision. Static fields run during class evaluation, base instance fields run when the instance binding is established before parameter defaults and the constructor body, and derived instance fields run immediately after a successful `super` return. The plan distinguishes explicit constructors, implicit base constructors, and implicit derived constructors that forward all arguments. It deliberately does not choose target storage or synthesize target syntax; Haxe and Rust consume those facts in later lowering, and the current IR must first gain an explicit super-constructor operation before a backend can safely rewrite an explicit derived constructor.
