...
4. **The three defects `flight-cpp` named in this round.** `captured referent mutation of ctx` needs the compiler's reference representation for a captured binding, and the upstream example manifests need the Web host package declared.
   ...
   The third — a local array literal over native-binding property reads losing its element type (`const restored = [ctx.globalAlpha, ctx.lineWidth]` emitting `flight::Array<auto`) — **was not reproduced here**, and that is worth recording so the next attempt does not start where this one did. An `externalBindings` manifest carrying the same two number members on the same value binding emits `flight::Array<double>` correctly in _both_ the const-binding and the returned forms, so the trigger is not the array literal, not the const binding, and not the externality of the member by itself. What remains different about the real case is that the members come from a _profile_ binding for a DOM type rather than a manifest binding for a locally declared interface. Reproduce against the actual Canvas profile before changing anything.

5. **Intersections, now the largest blocked count in the corpus.** 11 direct modules block 114 — `@flighthq/node` holds four of them (`boundsRectangle`, `hierarchy`, `nodeOrderList`, `traversal`), with the rest in `collision`, `physics2d`, `physics3d`, `scene3d` and `types`. It is the highest-leverage family left and the one least understood: `NodeOf<Traits> = Node<Traits> & NoInfer<Traits>` looks like the trigger, but that alias alone emits in every shape tried so far, so the refusal needs context this reproduction does not carry. The positions are in the ledger — start from `packages/node/src/traversal.ts:19` and read the declaration rather than guessing at it.
...
## The plain-object host seam: two compile-level defects

Flight `7e2fc7df5378313ceaf21ba8d6ef39cc18a6f4ad` removed `extends Entity` from the host capability interfaces (`2b8c7656`) and builds capabilities as plain objects (`5a251b69`). The shape is modeled end to end in `cppCompilerBackend.test.ts` — `types/HostGl.ts` declares `HostGlCapability`, `host-web/webGlHost.ts` exports a plain object satisfying the optional-member bag, and `app/appWindow.ts` receives the capability as a parameter and calls through it — and the emitted headers were handed to `g++` 15.2.0, which **does not compile them**. Two defects, both general:

1. **A method-bearing interface reaches emission as a structural row with no row member for its methods.** The consumer parameter becomes `StructuralRef<RowReadonly<RowOf<Ref<HostGlCapability>>>>` and the call becomes `row_get<RowKey<"acquire">>(host_gl)(surface)`, which fails `static_assert: structural row schema does not contain the requested property`. The interface is a bag of functions; the row schema built from it carries none of them.
2. **An optional capability member checks presence by comparing a reference against `flight::undefined`.** `HostGlCapabilities { readonly context?: HostGlCapability }` emits `if ((group.context != flight::undefined))` where `group.context` is `Ref<HostGlCapability>`, which has no such operator. The presence of an optional _object_ member needs the same projection the erased-value lane already has, not a sentinel comparison.

Recorded as findings rather than fixed here: both are representation changes in the C++ backend, and the regression test pins only the half that is correct — the cross-package forward declarations, the includes, and the qualification, all of which the shape does exercise. What it must not do is pin the two failures above, which is why the test asserts the declarations and the call rather than the row spelling.

The model needed correcting on the way, which is worth recording: a first version supplied `importer` identities on the module-resolution edges, and with those the resolution found no targets, so the consumer emitted bare `Surface` with no include and no forward declaration — which looks exactly like a missing-declaration defect and is not one. The resolvable form is an edge with `specifier` and no importer, matching the other multi-module tests in the file. **A shape that fails to compile is not yet a compiler defect until the harness that failed it is itself resolved.**
