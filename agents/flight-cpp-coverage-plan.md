...
**1a alone moves nothing, and that is worth knowing before starting.** Every one of the sixteen packages that pass a concrete argument also depends on `@flighthq/node` and imports its contract, and the ledger already shows `node.ts` refusing 24 modules directly through dependency edges. So the sixteen, once 1a clears their own conditional, become dependency-refused on `node.ts` in its place; the closure only opens when 1b lands as well. Read the split as "1a is the prerequisite, 1b is the unlock", not as two independent halves whose first half pays.
