...
Even with 1a resolved, 1b is load-bearing: substituting the conditional for a syntactic indexed access still refuses, because a generic `Omit<T, 'data'>` has no C++ row projection to lower to. `flight-cpp`'s `structural_ref.hpp` supplies `RowOf`, `RowMerge`, `RowPartial`, `RowReadonly`, and `RowWritable` but no "this row without this key" primitive, so 1b needs a decision between a downstream row-projection primitive and a compiler-side policy that materialises per instantiation. Concrete instantiations already work — `Partial<Omit<Node2D, 'data'>>` at a use site materialises a nominal row — so the decision is only about the generic form.
