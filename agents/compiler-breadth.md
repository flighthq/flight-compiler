...
Only what the duplication evidence actually supports: **deterministic text order** (11 copies across four packages) and **portable path form** (10 sites across five, in two disagreeing spellings, where the move also decides the machine-dependence). Nothing target-specific, nothing that reads a checkout.
...
The drift has already started: `fileSystemWorkspaceSource.test.ts` sorts with `localeCompare` while all eleven production copies sort by code unit. It is only a test's assertion order today, and it is the first spelling of a rule that exists in eleven places and is written down in none.
