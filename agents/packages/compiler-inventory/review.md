...
- **No incrementality.** Every run constructs a full TypeScript program and walks every package. This is the slowest operation in the repository — the one fixture that builds a real program takes 12.8s under coverage instrumentation after tuning — and nothing is cached across runs.
