...
- **Order independence.** Preferred spellings are reserved before suffixing, so input order cannot choose which declaration wins — the property that makes the allocation reproducible across runs. It is strong enough that ordering candidates by preferred spelling was provably unobservable; mutation found the term and it was removed rather than tested.
