...
**early — 42/100.** The framework is stronger than the score: pass identity and order are explicit, input is isolated, output is verified, declared idempotence is checked, failures name the responsible pass, and every branch is exercised. The domain is nevertheless early because only one of the neutral transformations already named by backend refusals exists.
...
- Verification after every transform and a declared idempotence contract. Present.
...
- **Verified pass execution.** Duplicate names and reversed declared order fail before work begins. Each output passes its verifier, and an idempotent pass is applied again and compared structurally.
