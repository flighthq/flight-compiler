...
- **Deterministic ordering, locale-independent.** Modules and files sort through the shared `compiler-canonical-form` code-unit primitive, so the environment's locale cannot reorder output.
