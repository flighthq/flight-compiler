...
- **Deterministic ordering, host-independent.** Modules and files sort by explicit code-unit comparison after the locale-ordering pass, so a machine's locale cannot reorder output.
