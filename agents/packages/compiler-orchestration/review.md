...
- **Deterministic ordering, host-independent.** Modules and files sort through the shared `compiler-ordering` code-unit primitive, so a machine's locale cannot reorder output.
