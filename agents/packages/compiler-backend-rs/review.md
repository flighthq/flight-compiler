...
- **`FlightCallback` and the opaque host value are named without a contract.** Emitted code references symbols whose shape is not defined anywhere in this repository, which is intentional — the runtime lives downstream — but the contract they must satisfy is not written down here either.
