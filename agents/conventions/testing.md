...
**Read a new fixture's generated output before committing it.** A baseline asserts that the emitted text is correct, so committing one without reading it blesses whatever the compiler does today. This is not hypothetical: the `classShape` fixture landed pinning `this_.step`, which is not valid Haxe — `this` had been passed through the keyword-mangling path. The fixture still did its job, because the next change to that code path produced a reviewable diff, but a defect sat blessed in the tree until then. Adding a fixture is two steps: generate it, then review the output as if someone else had written it. Where the output is wrong, do not commit the fixture — report the defect and add the fixture once the behavior is decided.
