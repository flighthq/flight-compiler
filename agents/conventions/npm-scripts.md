...
- **Omit the subject** → run that action across every subject. `check` is the whole-repository static sweep; `test` is the whole unit suite.
...
`ci` is `clean` plus `verify`: the complete sweep from a cold tree.
