...
- [ ] Extend all typed-array implementations with construction from `flight::ArrayBuffer`, a `buffer` view, and `byte_offset`. Views over one buffer must observe one another's writes; copying the input bytes is not compatible with TypeScript semantics.
...
The focused package probes also expose work that must remain in `flight-compiler`: package-graph type evidence for isolated ABC, Shell, and XML probes; for-of destructuring with imported element types; unbounded iterable spread; and imported anonymous-object evidence. Synchronous executable top-level control flow now uses the compiler's ordered module-initialization lane; top-level `await` and nested module-scoped `var` remain explicit refusals. Do not compensate for compiler gaps with permissive runtime types or downstream source rewriting. Once compiler support lands, rerun the downstream adoption gates above and add only the runtime surface the emitted contract actually requires.
