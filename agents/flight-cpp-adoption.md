...
- [ ] **A typed-array element write needs a converting write spelling.** `flight::TypedArray::element(double)` returns a raw `Value&`, so the generated `values.element(0.0) = 300.0;` assigns a `double` straight into element storage. For a value outside the element's range that conversion is undefined behaviour rather than a wrap, and the toolchains disagree because of it: a focused reproduction gave GCC 14 `[255,0,2,1,58]`, Clang 18 `[0,0,2,1,58]`, and MSVC `[255,0,2,1,58]` where ECMAScript requires `[44,255,2,1,58]`, while this checkout's GCC happens to produce the ECMAScript answer. None of those choices is the standard's; the portable statement is only that the conversion must be the runtime's, because the compiler's typed-array element writes are correct today exactly where a toolchain's undefined behaviour coincides with ECMAScript.

  The runtime already owns the right conversion: `TypedArray::convert_element<Source>` applies the ECMAScript rules, clamps for `Uint8Clamped`, and maps NaN and infinities the way the language does. It is `private` and reachable only from the constructors and the iterator copy, so nothing the compiler emits can use it. The ask is one public spelling for an indexed write, and the spelling to match is the Rust runtime's: this compiler already lowers an indexed read and write for Rust as `get_index(index)` and `set_index(index, value)`, so `Value get_index(double index)` and `void set_index(double index, Source&& value)` storing `convert_element(std::forward<Source>(value))` would let one lowering serve both targets instead of the C++ emitter keeping a second spelling for the same operation. Making `convert_element` public, or returning a proxy from `element(double)` whose `operator=` converts, would also close the hole; the pair is the one that matches what the other target already compiles. Semantics requested: the same conversion a construction applies, so a write and a construction agree for every element type; one conversion per write; no clamping on the integral types that do not clamp. This is not an MSVC-only divergence: with the compile lane scoped, the C++ oracle fails the same answer on GCC, Clang, and AppleClang as well, so the fault is the target's rather than the runner's.
