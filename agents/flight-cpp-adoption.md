...
- [ ] Replace the two shaded-material cache `unknown` values with a concrete shared source type, or define an explicit type-erased value and checked typed-view contract at the Flight source/runtime boundary. The unchecked `as WeakMap<K, V>` assertions do not supply runtime recovery evidence, so these two fields remain the sole WeakMap source-boundary blocker after the runtime and host-policy work lands.

No compiler-only relaxation is sound for the two `unknown` values: spelling them as `void` or `flight::Any` without a checked source/runtime contract would fabricate recovery semantics that the TypeScript assertions do not prove.
