...
- [ ] Provide a row projection that removes one named key, so a generic `Omit<Row, 'key'>` has a C++ spelling. `structural_ref.hpp` supplies `RowOf`, `RowMerge`, `RowPartial`, `RowReadonly`, and `RowWritable` but nothing that subtracts a key; a concrete `Omit<Row, 'key'>` already materialises a nominal row at its use site, so only the generic form is missing. `@flighthq/types`'s `PartialNode<T>` is `{ … } & Partial<Omit<T, 'data'>>`, and it blocks 17 modules directly and up to 446 through its dependents.
