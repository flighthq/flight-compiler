...
describe('createCompilerRuntimeExternalConstructorAbiPlanHaxe', () => {
  it('elects exact fixed arities under the versioned constructor ABI contract', () => {
    ...
    expect(plan.constructors).toHaveLength(44);
    ...
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'Function', space: 'value' },
      fixedArgumentCounts: [1],
    });
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'OffscreenCanvas', space: 'value' },
      fixedArgumentCounts: [2],
    });
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'DataView', space: 'value' },
      fixedArgumentCounts: [1, 2, 3],
    });
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
    ...
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'Intl.Segmenter', space: 'value' },
      fixedArgumentCounts: [0, 1, 2],
    });
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'DOMException', space: 'value' },
      fixedArgumentCounts: [0, 1, 2],
    });
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
    ...
    expect(plan.constructors).toContainEqual({
      dynamicArguments: false,
      externalSymbol: { sourceName: 'SharedArrayBuffer', space: 'value' },
      fixedArgumentCounts: [1],
    });
    expect(
  ...
  it('creates independent plan records and nested arity lists', () => {
    ...
    expect(second.constructors).toHaveLength(44);
