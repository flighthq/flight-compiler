...
export function analyzeFlightPackageImports(
  ...
  for (const file of walkProductionTypeScriptFiles(path.join(packageDirectory, 'src'), workspace)) {
    ...
    for (const record of analyzeFlightSourceImports(source, portableSource)) {
      records.set(JSON.stringify([record.source, record.specifier, record.kind, record.typeOnly]), record);
    }
  }
  return [...records.values()].sort(comparePackageImportRecords);
}

export function analyzeFlightSourceImports(
