...
describe('createCompilerLoweringPassAwaitConditionHoisting', () => {
  ...
  it('binds a leading await passed as the first argument of a returned local call', () => {
    const output = lowerIrModuleWithCompilerPasses(
      lower(`
        function wrap(value: number): number { return value; }
        export async function load(task: Promise<number>): Promise<number> {
          return wrap(await task);
        }
      `),
      [createCompilerLoweringPassAwaitConditionHoisting()],
    );
    const declaration = output.declarations.find(
      (candidate) => candidate.kind === 'function' && candidate.binding.name === 'load',
    );
      ...
      body: [
        {
          ...
          statements: [
            { declarations: [{ binding: { name: 'awaitValue' }, initializer: { kind: 'await' } }] },
            {
              expression: { arguments: [{ kind: 'identifier' }], kind: 'call' },
              kind: 'return',
            },
          ],
        },
  ...
  it('binds a leading await passed to a returned ambient constructor', () => {
    const output = lowerIrModuleWithCompilerPasses(
      lower(`
        export async function load(task: Promise<ArrayBuffer>): Promise<Uint8Array> {
          return new Uint8Array(await task);
        }
      `),
      [createCompilerLoweringPassAwaitConditionHoisting()],
    );
      ...
      body: [
        {
          ...
          statements: [
            { declarations: [{ binding: { name: 'awaitValue' }, initializer: { kind: 'await' } }] },
            {
              expression: { arguments: [{ kind: 'identifier' }], kind: 'new' },
              kind: 'return',
            },
          ],
        },
  ...
  it('hoists awaited constructor arguments inside nested async functions', () => {
    const output = lowerIrModuleWithCompilerPasses(
      lower(`
        export function create(): (blob: Blob) => Promise<Uint8Array> {
          return async (blob: Blob): Promise<Uint8Array> => {
        ...
        body: [
          {
            ...
            statements: [
              { declarations: [{ binding: { name: 'awaitValue' }, initializer: { kind: 'await' } }] },
              { expression: { arguments: [{ kind: 'identifier' }], kind: 'new' }, kind: 'return' },
            ],
          },
        ],
        kind: 'function',
