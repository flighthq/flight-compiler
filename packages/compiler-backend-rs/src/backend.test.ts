...
import { emitRustModule } from './index.js';
...
describe('Rust emission', () => {
  it('emits numeric enums and rejects string enum representation', () => {
    ...
    expect(emitRustModule(numeric.module).contents).toContain('D = 9,');
    expect(() => emitRustModule(strings.module)).toThrow('requires integer discriminants for Rust');
  });
  ...
  it('rejects module facades, operators, and switch fallthrough without target lowering', () => {
    ...
    expect(() => emitRustModule(barrel.module)).toThrow('module-facade lowering');
