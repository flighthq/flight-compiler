...
export function getIrHomogeneousTupleElementTypeCpp(type: Readonly<IrType>): Readonly<IrType> | undefined {
  ...
  return type.elements.every(
    (element) =>
      !element.optional && !element.rest && normalizeCompilerStructuralValueCanonical(element.type) === identity,
  )
    ? first.type
    : undefined;
}
