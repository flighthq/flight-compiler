...
export function omitKeys<Key extends keyof Provider>(): Omit<Provider, Key> {
  ...
  it('assigns a nullable NodeOf parent to a nullable Node loop variable', () => {
    ...
    expect(emitted).toContain('flighthq_node::get_node_parent<Traits>(');
  ...
  it('passes a nullable spatial parent into a dual-sentinel generic parameter without changing null to undefined', () => {
    const moduleResolution: CompilerModuleResolutionPlan = {
      edges: [
        {
          specifier: '@flighthq/types/contract',
    ...
    expect(results.flatMap((result) => result.diagnostics)).toEqual([]);
    expect(emitted).toContain('compute_node_bounds_rectangle<Traits>');
    expect(emitted).toContain('auto contextual_union_source = flighthq_node::get_node_parent<Traits>(source);');
    expect(emitted).toContain('if (!contextual_union_source.has_value())');
    expect(emitted).not.toContain('get_node_parent<Traits>(source).value()');
    expect(emitted).toContain('flight::null');
    expect(emitted).not.toContain('flight::undefined');
  });

  it('refuses widening an unrelated nullable row into a dual-sentinel row', () => {
    const result = lower(
      'unrelated-dual-sentinel.ts',
      `interface Source { readonly source: number }
       interface Target { readonly target: number }
       export function forward(source: Source | null): Target | null | undefined {
         return source;
       }`,
    );
  ...
  it('reassigns a narrowed same-module generic parent while collecting ancestors', () => {
    const moduleResolution: CompilerModuleResolutionPlan = {
      edges: [
        {
          specifier: '@flighthq/types/contract',
  ...
  it('returns the concrete overload result from findNodeByName', () => {
    ...
    expect(results.flatMap((result) => result.diagnostics)).toEqual([]);
    expect(emitted).toContain('std::optional<flighthq_types::NodeOf<Traits>> find_node_by_name');
    expect(emitted).toContain('return find_node<Traits>');
    expect(emitted).toContain('node->name');
    expect(emitted).not.toContain('flight::Any');
  });
